...
The `chain_plugin` is a core plugin required to process and aggregate chain data on an TLKIO node.
...
These can only be specified from the `nodtlk` command-line:
...
```console
Config Options for tlkio::chain_plugin:
