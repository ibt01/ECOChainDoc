...
The `http_plugin` is a core plugin supported by both `nodtlk` and `ktlkd`. The plugin is required to enable any RPC API functionality provided by a `nodtlk` or `ktlkd` instance.
...
```sh
# command-line
nodtlk ... --plugin tlkio::http_plugin [options]
 (or)
ktlkd ... --plugin tlkio::http_plugin [options]
```
...
Config Options for tlkio::http_plugin:
  --unix-socket-path arg                The filename (relative to data-dir) to 
                                        ...
                                        blank to disable (=ktlkd.sock for ktlkd)
  --http-server-address arg (=127.0.0.1:8888 for nodtlk)
