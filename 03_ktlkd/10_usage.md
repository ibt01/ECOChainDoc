---
content_title: Ktlkd Usage
---

[[info | Recommended Usage]]
| For most users, the easiest way to use `ktlkd` is to have `cltlk` launch it automatically. Wallet files will be created in the default directory (`~/tlkio-wallet`).

## Launching ktlkd manually

`ktlkd` can be launched manually from the terminal by running:

```sh
ktlkd
```

By default, `ktlkd` creates the folder `~/tlkio-wallet` and populates it with a basic `config.ini` file.  The location of the config file can be specified on the command line using the `--config-dir` argument.  The configuration file contains the HTTP server endpoint for incoming HTTP connections and other parameters for cross-origin resource sharing.
...
By default, `ktlkd` is set to lock your wallet after 15 minutes of inactivity. This is configurable in the `config.ini` by setting the timeout seconds in `unlock-timeout`. Setting it to 0 will cause `ktlkd` to always lock your wallet.

## Stopping ktlkd

The most effective way to stop `ktlkd` is to find the ktlkd process and send a SIGTERM signal to it.

## Other options

For a list of all commands known to `ktlkd`, simply run it with no arguments:
...
Config Options for tlkio::http_plugin:
  --unix-socket-path arg (=ktlkd.sock)  The filename (relative to data-dir) to
...
Config Options for tlkio::wallet_plugin:
