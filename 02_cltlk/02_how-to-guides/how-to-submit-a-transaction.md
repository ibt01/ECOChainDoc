...
[[info]]
| Be aware that if a clear text `data` field is used, cltlk need to fetch copies of required ABIs using `nodtlk` API. That operation has a performance overhead on `nodtlk`
...
```sh
cltlk push transaction TRX_FILE.json
```

* Submit a transaction from a JSON:

```sh
cltlk push transaction JSON
```
